...
# TODO what to clal this?
class KLqp(GANInference):
  ...
  def build_loss_and_gradients(self, var_list):
    ...
    loss_d = tf.nn.sigmoid_cross_entropy_with_logits(
        labels=tf.ones_like(r_psample), logits=r_psample) + \
        tf.nn.sigmoid_cross_entropy_with_logits(
            labels=tf.zeros_like(r_qsample), logits=r_qsample)
...
# TODO move to utility; to be used across
def log_loss(psamples, qsamples):
  """Point-wise log loss."""
...
def hinge_loss(psamples, qsamples):
